...
static const char SCCSid[] =
    "@(#)sgdp4.c       3.03 (C) 1995 psc SatLib: Orbital Model";
...
init_sgdp4 (orbit_t *orb)
{
  ...
  if (imode == SGDP4_NEAR_NORM) {
	...
	* ((real) 221.0 * aodp + (real) 31.0 * s4) * c1;
    ...
    t4cof = (real) 0.25
	* ((real) 3.0 * d3 + c1 * ((real) 12.0 * d2 + (real) 10.0 * c1sq));
    t5cof = (real) 0.2
	* ((real) 3.0 * d4 + (real) 12.0 * c1 * d3 + (real) 6.0 * d2 * d2
...
#define ts tsince
  ...
  if (withvel) {
	...
	- temp2 * temp1 * x1mth2 * sin2u) * (XKMPER / AE * XMNPDA / 86400.0); /* Into km/sec */
